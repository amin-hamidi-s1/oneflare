...
| Variable | Contains |
|----------|----------|
| `API_URL` | Full console base URL, e.g. `https://usea1-acme.sentinelone.net` |
| `API_TOKEN` | Console User (personal) API token (generated in Settings → Users → API Token) |
...
### Step 1 — Validate `API_URL` (no auth required)

```
GET {API_URL}/web/api/v2.1/system
```

This endpoint requires no authentication and always returns `200` with `{"data": {"health": "ok"}}`
when the console is reachable. A non-200 response or a network error means `API_URL` is wrong
or the console is unreachable.

### Step 2 — Validate `API_TOKEN` (auth + permission check)

```
GET {API_URL}/web/api/v2.1/hyper-automate/api/public/workflows?limit=1
...
async function validateCredentials(apiUrl, apiToken) {
  ...
  if (!sysRes.ok) throw new Error(`Console unreachable (${sysRes.status}). Check API_URL.`);
  ...
  if (authRes.status === 401) throw new Error('API_TOKEN is invalid or expired.');
  if (authRes.status === 403) throw new Error('API_TOKEN lacks Hyper Automate.view permission.');
...
Uses the `API_URL` and `API_TOKEN` environment variables. Always call `validateCredentials()`
(defined above) before any workflow operation.

```javascript
const apiUrl   = process.env.API_URL;    // e.g. https://usea1-acme.sentinelone.net
const apiToken = process.env.API_TOKEN;  // Service User API token
